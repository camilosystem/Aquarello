...
import { DomiciliarioDetailClient } from '@/components/operador/domiciliario-detail-client'

interface PageProps {
  params: Promise<{ id: string }>
}

export default async function DomiciliarioDetailPage({ params }: PageProps) {
  ...
  const { data: domiciliario, error } = await supabase
    .from('profiles')
    .select('*')
    .eq('id', id)
  ...
  if (error || !domiciliario) notFound()
