'use client'

import { useState, useTransition, useEffect } from 'react'
import { createClient } from '@/lib/supabase/client'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { Textarea } from '@/components/ui/textarea'
import { Badge } from '@/components/ui/badge'
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from '@/components/ui/dialog'
import { Separator } from '@/components/ui/separator'
import { toast } from 'sonner'
import { CreditCard, Banknote, CheckCircle2, Loader2, AlertCircle } from 'lucide-react'
import { registerManualPaymentAction, type ManualPaymentMethod } from '@/app/operador/tickets/payment-actions'
import { formatUSD } from '@/lib/types'

interface Props {
  orderId: string
  orderAmount?: number | null
  buyerEmail?: string | null
  onPaid?: () => void
}

type Method = 'efectivo' | null

const METHODS = [
  {
    key: 'efectivo' as const,
    label: 'Cash',
    desc: 'Record a payment received in cash',
    icon: Banknote,
    color: 'bg-green-50 border-green-200 text-green-700 hover:bg-green-100',
  },
]

export function PaymentButton({ orderId, orderAmount, onPaid }: Props) {
  const [open, setOpen] = useState(false)
  const [method, setMethod] = useState<Method>(null)
  const [amount, setAmount] = useState(String(orderAmount ?? ''))
  const [notes, setNotes] = useState('')
  const [paidSoFar, setPaidSoFar] = useState(0)
  const [isPending, startTransition] = useTransition()

  const total = orderAmount ?? 0
  const remaining = Math.max(0, total - paidSoFar)
  const parsedAmount = parseInt(amount) || 0
  const isFullPayment = parsedAmount > 0 && parsedAmount >= remaining
  const isFullyPaid = total > 0 && paidSoFar >= total

  // Load existing payments to calculate how much has been paid
  useEffect(() => {
    const supabase = createClient()
    if (!supabase) return
    supabase
      .from('payments')
      .select('amount')
      .eq('order_id', orderId)
      .eq('status', 'completado')
      .then(({ data }) => {
        const paid = data?.reduce((sum, p) => sum + (p.amount ?? 0), 0) ?? 0
        setPaidSoFar(paid)
      })
  }, [orderId])

  const resetDialog = () => {
    setMethod(null)
    setAmount(String(remaining > 0 ? remaining : (orderAmount ?? '')))
    setNotes('')
  }

  const handleClose = () => { setOpen(false); resetDialog() }

  const handleOpen = () => {
    setAmount(String(remaining > 0 ? remaining : (orderAmount ?? '')))
    setOpen(true)
  }

  // ── Cash ─────────────────────────────────────────────────────────────────
  const handleManualPay = () => {
    if (parsedAmount <= 0) { toast.error('Enter a valid amount'); return }
    startTransition(async () => {
      const result = await registerManualPaymentAction({
        order_id: orderId,
        amount: parsedAmount,
        method: method as ManualPaymentMethod,
        notes,
        mark_as_paid: isFullPayment,
      })
      if (result.ok) {
        if (isFullPayment) {
          toast.success('Payment recorded in full')
          setPaidSoFar(paidSoFar + parsedAmount)
          handleClose()
          onPaid?.()
        } else {
          toast.success(`Partial payment of ${formatUSD(parsedAmount)} recorded`)
          setPaidSoFar(prev => prev + parsedAmount)
          setAmount(String(Math.max(0, remaining - parsedAmount)))
          setNotes('')
        }
      } else {
        toast.error(`Error: ${result.error}`)
      }
    })
  }

  if (isFullyPaid) {
    return (
      <Badge className="bg-green-100 text-green-800 border border-green-200 gap-1 py-1 px-2">
        <CheckCircle2 className="h-3.5 w-3.5" /> Paid
      </Badge>
    )
  }

  return (
    <>
      <Button
        size="sm"
        variant="outline"
        className="border-emerald-200 text-emerald-700 hover:bg-emerald-50"
        onClick={handleOpen}
      >
        <CreditCard className="mr-2 h-4 w-4" />
        {paidSoFar > 0 ? 'Record Partial Payment' : 'Record Payment'}
      </Button>

      <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); else setOpen(true) }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Record Payment</DialogTitle>
          </DialogHeader>

          {/* Step 1 — choose method */}
          {!method && (
            <div className="space-y-2 py-2">
              <p className="text-sm text-muted-foreground">Select a payment method:</p>
              {METHODS.map(m => (
                <button
                  key={m.key}
                  onClick={() => setMethod(m.key)}
                  className={`w-full flex items-center gap-3 p-3 rounded-lg border text-left transition-colors ${m.color}`}
                >
                  <m.icon className="h-5 w-5 shrink-0" />
                  <div>
                    <p className="font-medium text-sm">{m.label}</p>
                    <p className="text-xs opacity-80">{m.desc}</p>
                  </div>
                </button>
              ))}
            </div>
          )}

          {/* Step 2 — form */}
          {method && (
            <div className="space-y-4 py-2">
              <button
                onClick={() => setMethod(null)}
                className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1"
              >
                ← Change method
              </button>
              <Separator />

              {/* Amount field */}
              <div className="space-y-2">
                <Label>Amount (USD)</Label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">$</span>
                  <Input
                    type="number"
                    min="0"
                    className="pl-7"
                    value={amount}
                    onChange={e => setAmount(e.target.value)}
                  />
                </div>

                {/* Ticket breakdown */}
                {total > 0 && (
                  <div className="rounded-lg border p-3 space-y-1.5 text-sm bg-muted/40">
                    <div className="flex justify-between text-muted-foreground">
                      <span>Ticket total:</span>
                      <span className="font-medium text-foreground">{formatUSD(total)}</span>
                    </div>
                    {paidSoFar > 0 && (
                      <div className="flex justify-between text-emerald-700">
                        <span>Already paid:</span>
                        <span className="font-medium">{formatUSD(paidSoFar)}</span>
                      </div>
                    )}
                    <div className="flex justify-between border-t pt-1.5 font-medium">
                      <span>Balance due:</span>
                      <span>{formatUSD(remaining)}</span>
                    </div>
                  </div>
                )}

                {/* Confirmation indicator */}
                {parsedAmount > 0 && isFullPayment && (
                  <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-2.5 text-emerald-700">
                    <CheckCircle2 className="h-4 w-4 shrink-0" />
                    <span className="text-sm font-medium">Full amount confirmed</span>
                  </div>
                )}
                {parsedAmount > 0 && !isFullPayment && parsedAmount < remaining && (
                  <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 p-2.5 text-amber-700">
                    <AlertCircle className="h-4 w-4 shrink-0" />
                    <span className="text-sm">Partial payment · {formatUSD(remaining - parsedAmount)} remaining</span>
                  </div>
                )}
              </div>

              {/* Notes */}
              <div className="space-y-1.5">
                <Label className="text-sm">Notes (optional)</Label>
                <Textarea
                  placeholder="E.g.: paid with a $50 bill"
                  value={notes}
                  onChange={e => setNotes(e.target.value)}
                  rows={2}
                />
              </div>
            </div>
          )}

          {method && (
            <DialogFooter>
              <Button variant="outline" onClick={handleClose} disabled={isPending}>
                Cancel
              </Button>
              <Button
                onClick={handleManualPay}
                disabled={isPending || parsedAmount <= 0}
                className={isFullPayment ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-amber-500 hover:bg-amber-600'}
              >
                {isPending
                  ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving…</>
                  : isFullPayment
                    ? <><CheckCircle2 className="mr-2 h-4 w-4" />Confirm Full Payment</>
                    : <><CheckCircle2 className="mr-2 h-4 w-4" />Record Partial Payment</>}
              </Button>
            </DialogFooter>
          )}
        </DialogContent>
      </Dialog>
    </>
  )
}
